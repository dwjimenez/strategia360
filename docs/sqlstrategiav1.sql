if exists (select 1
   ...
   where r.fkeyid = object_id('dbo.IntencionVotoOpcion') and o.name = 'FK_INTENCIO_REFERENCE_INTENCIO')
alter table dbo.IntencionVotoOpcion
   drop constraint FK_INTENCIO_REFERENCE_INTENCIO
go
...
alter table dbo.IntencionVotoOpcion
   add constraint FK_INTENCIO_REFERENCE_INTENCIO foreign key (IdIntencionVotoGrupo)
      references dbo.IntencionVotoGrupo (IdIntencionVotoGrupo)
go
